...
#define __DEFAULT_BUF_SIZE 1024
	...
	bool W3Client::Connect(	const char *szaddress,
		...
		try{
			...
			if(!_hOpen || _hOpen==INVALID_HANDLE_VALUE)
			{
				...
				ustring strError;
				strError.printf( 1000, _T("OpenInternet() failed, error %d.../n"), dwError );
				throw strError.c_str();
			}

			// MDM
			// Set any requested timeouts.
			if ( m_nConnectTimeoutMS	> 0 ) ::InternetSetOption( _hOpen, INTERNET_OPTION_CONNECT_TIMEOUT, &m_nConnectTimeoutMS	, sizeof(m_nConnectTimeoutMS	) );
			if ( m_nSendTimeoutMS		> 0 ) ::InternetSetOption( _hOpen, INTERNET_OPTION_SEND_TIMEOUT	, &m_nSendTimeoutMS		, sizeof(m_nSendTimeoutMS		) );
			if ( m_nReceiveTimeoutMS	> 0 ) ::InternetSetOption( _hOpen, INTERNET_OPTION_RECEIVE_TIMEOUT, &m_nReceiveTimeoutMS	, sizeof(m_nReceiveTimeoutMS	) );

			switch(t) {
			case w3ftp:
				_hConnection=::InternetConnect(	_hOpen,
																		szaddress, (nport? (INTERNET_PORT)nport: INTERNET_DEFAULT_FTP_PORT),
																		szuser, szpassword,
			...
			case w3http:
				_hConnection=::InternetConnect(	_hOpen,
																		szaddress, (nport? (INTERNET_PORT)nport: INTERNET_DEFAULT_HTTP_PORT),
																		szuser, szpassword,
			...
			case w3https:
				_hConnection=::InternetConnect(	_hOpen,
																		szaddress, (nport? (INTERNET_PORT)nport: INTERNET_DEFAULT_HTTPS_PORT),
																		szuser, szpassword,
																		INTERNET_SERVICE_HTTP,
																		0,
																		NULL);
		...
		// MDM 
		// Fuck it, we're getting bad strings back here.
		// But skipping this causes other problems?
	...
	unsigned long W3Client::AllocMultiPartsFormData(unsigned char *&buf, const char *szboundry){
		...
		// TO DO
		// buf may be zero here if the allocation fails!
		ASSERT( buf );
	...
	void W3Client::FreeMultiPartsFormData(unsigned char*& buf)
	{ 
		// MDM Getting exceptions here...
		// How about we fix the PROBLEM?  :P
		if ( buf )
		{
			::HeapFree(::GetProcessHeap(), 0, buf);
			buf = 0;
	...
	bool W3Client::RequestPost2(const char *szuri, const char *szref /*=NULL*/)
	{
		...
		// MDM IS THIS STANDARD?
		// static LPCTSTR szContentType="Content-Type: multipart/form-data; boundary=--MULTI-PARTS-FORM-DATA-BOUNDARY\r\n";
		// static LPCTSTR szContentType="Content-Type: multipart/form-data; boundary=MDM-MY-MULTI-PARTS-FORM-DATA";
		...
		if(
			...
			( 
				_hRequest, 
				szMultiPartContentType, 
				strlen(szMultiPartContentType), 
		...
		len = AllocMultiPartsFormData( buf, szMultiPartBoundaryTag );
		...
		// MDM 
		// OK, we may be posting a really large buffer here.
		// According to MSDN, that's OK but we have to do it
		// by repeatedly calling InternetWriteFile() with 1024-byte
		// chunks.  From the sample code at 
		//
		...
		// 		}
		//
		// Wow that's shitty code.  WRONG code.  To quote Anthony, "the mind reels...".
		...
		InternetBufferIn.dwHeadersTotal = 0;
		InternetBufferIn.lpvBuffer = NULL;                
		InternetBufferIn.dwBufferLength = 0;
		InternetBufferIn.dwOffsetLow = 0;
		InternetBufferIn.dwOffsetHigh = 0;
		
		// MDM According to the docs, we NEED to set this, and we must post exactly this amount.
		InternetBufferIn.dwBufferTotal = len; // This is the only member used other than dwStructSize
	
		// MDM
		// I don't know where the HSR_INITIATE comes from, but the current
		// docs say "This parameter is reserved and must be set to zero".
		if
		(
			!::HttpSendRequestEx(
				_hRequest, 
		...
		// --------------------
		// Write out in chunks.
		// NOTE: this is written as directed in the BigPost sample from
		//
		//		http://support.microsoft.com/kb/177188/EN-US/
		//
		// Only not as shitty.  :P
		// --------------------
		const int cnInternetWriteFileMaxChunkSize = 1024;
		BOOL bUploadOK = TRUE;
		int nTotalBytesOut = 0;
		int nNumberOfWrites = ( len - 1 ) / cnInternetWriteFileMaxChunkSize + 1;
		int nA;
		for ( nA = 0; nA < nNumberOfWrites && bUploadOK; nA++ )
		{
			int nBytesWritten = 0;
			int nChunkSize = std::min( (int)len - nTotalBytesOut, cnInternetWriteFileMaxChunkSize );
			if ( bUploadOK = ::InternetWriteFile( _hRequest, buf + nTotalBytesOut, nChunkSize, (LPDWORD)&nBytesWritten ) )
			{
				ASSERT( nChunkSize == nBytesWritten );
				nTotalBytesOut += nBytesWritten;
			}
		}
		ASSERT( nTotalBytesOut == len );
		ASSERT( bUploadOK );
		// --------------------


		// Original, one-shot upload of data, fails at 3000 bytes or more.
		/*
		unsigned long nout=0;
		...
		// MDM
		// Make sure we wrote what we expected to.
		ASSERT( len == nout );
		*/
		...
		// -----------------------------------------------------------
		// MDM
		// THIS CALL IS LOCKING UP, it never returns.
		// -----------------------------------------------------------
		// 3) The try/catch doesn't seem to help...
		// -----------------------------------------------------------
		// 2) From InternetWriteFile() docs:
		//
		//		When the application is sending data, it must call 
		//		InternetCloseHandle to end the data transfer.
		//
		// It doesn't say anything about HttpEndRequest().
		// Let's try just removing this call?  No, then response is blank...
		// I did convert more CloseHandles to SafeClose, still broke...
		// -----------------------------------------------------------
		// 1) I don't know where the HSR_INITIATE comes from, but the current
		// docs say "This parameter is reserved and must be set to zero".
		// -----------------------------------------------------------
		try
		{
...
// MDM Let's make a SIMPLE ASYNC HTTP CLIENT REQUEST.
// All we want is a multipart-post HTTP request with a timeout.
