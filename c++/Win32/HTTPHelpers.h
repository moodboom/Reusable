...
// Keep these strings synched.
// NOTE: Orig string: "--MULTI-PARTS-FORM-DATA-BOUNDARY-"
const LPCTSTR szMultiPartBoundaryTag = _T("MDMMULTIPARTDATA");
const LPCTSTR szMultiPartContentType = _T("Content-Type: multipart/form-data; boundary=MDMMULTIPARTDATA\r\n");
#define __DEFAULT_BOUNDRY_TAG "MDMMULTIPARTDATA"
...
/////////////////////////////////////////////////////////////////
	...
	public:
		...
		: 
			...
			m_nConnectTimeoutMS( 10000 ),
			m_nSendTimeoutMS   ( 10000 ),
			m_nReceiveTimeoutMS( 10000 )
	...
	private:
		typedef struct __HTTP_ARG {
			...
			// MDM 3/1/06
			// ------------------------------------------------------------------
			// Embedded &'s in post data are making the receiving end puke.
			// I tried to wrap the value in quotes here, but that's a wasted
			// effort - the data is still truncated at the & on the receiving end.
			// Attempt to wrap data value in quotes:
			/*
			unsigned long length(){	return name.length()+value.length()+3; }	// 3 for the 3 chars in [name="value"]
			unsigned long dump(unsigned char *buf, unsigned long s){
				memcpy(buf, name.c_str(), name.length());
				memcpy(buf+name.length(), "=\"", 2);
				memcpy(buf+name.length()+2, value.c_str(), value.length());
				memcpy(buf+name.length()+2+value.length(), "\"", 1);
				return name.length()+value.length()+3;
			}
			*/
			// Original code:
			...
			// MDM
			// For multi-part (binary) form data .
			...
			unsigned long dump2(unsigned char *buf, unsigned long s, const char *boundry=__DEFAULT_BOUNDRY_TAG){
				...
				if(f){ // file
					...
					*/
					...
					// MDM
					// Do we want to start with '--' but not end with it?  Apparently so,
					// as tested with Firefox!!  Go figure.
					//
					// Do we need a trailing ';'?  No.  Apparently this code has been tested
					// and it's doing exactly the "right thing" (altho it sure looks like slop).
					sprintf( reinterpret_cast<char*>(buf),
								"--%s\r\n"
					...
					// MDM
					// Seriously, what kind of fucking moron code is this shit!!?
					l=	
							2+strlen(boundry)+2
							+38+name.length()+3 
							+38+2
	...
	// --------------------------------------------------
	// MDM I just want a timeout, so much to ask?  :P
	// Set these to 0 to disable.
public:
	void SetConnectTimeout	( int nTimeoutMS ) { m_nConnectTimeoutMS	= nTimeoutMS;	}
	void SetSendTimeout		( int nTimeoutMS ) { m_nSendTimeoutMS		= nTimeoutMS;	}			
	void SetReceiveTimeout	( int nTimeoutMS ) { m_nReceiveTimeoutMS	= nTimeoutMS;	}			

protected:
	int m_nConnectTimeoutMS;
	int m_nSendTimeoutMS   ;
	int m_nReceiveTimeoutMS;
	// --------------------------------------------------
	...
	protected:
		...
		// MDM Sloppy sloppy shit, I feel dirty using this code... sigh...
		bool SafeClose( HINTERNET& h )
		{
			if ( h == 0 )
				return false;

			bool bReturn = ( ::InternetCloseHandle( h ) != FALSE );
			h = 0;
			return bReturn;
		}
	};
...
// This doesn't compile - wtf is IWORKERTHREAD?
// Not gonna do it...
/*
...
class SimpleAsyncClient : public AsyncW3Client, public IWORKERTHREAD {
public:
	SimpleAsyncClient( unsigned int idx ):
		AsyncW3Client(), 
		IWORKERTHREAD(idx)
	{}
