...
// Let's not make things TOO obvious!
// #define subkeyfilename   "Blowfish.dat"
#define subkeyfilename   "consult.usr"
...
// Converted key to use far pointer.
// short InitializeBlowfish(char key[], short keybytes)
short InitializeBlowfish(
         unsigned char far * lpszKey,
         short keybytes)
{
   short          i;
   short          j;
   ...
   if (error == noErr) {
      ...
      for (i = 0; i < N + 2; ++i) {
         ...
         for (k = 0; k < 4; ++k) {
            ...
            // Converted key to use far pointer.
            // lpszKey must be unsigned!
            data = (data << 8) | lpszKey[j];
