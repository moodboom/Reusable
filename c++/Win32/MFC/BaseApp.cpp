...
// BaseApp.cpp
//
// This base class defines behaviors common across multiple 
// applications.
//-------------------------------------------------------------------//

#include "stdafx.h"

#include "PragmaMessages.h"						// For _DEBUGCODE() etc.
#include "BaseApp.h"

#include "AppRegistryData.h"						// This gives us App path and version.
#include "AppHelpers.h"								// For GetProgramPath(), GetProgramFilename(), GetShortFilename(), GetProgramName().
#include "DisplayMessage.h"						// For displaying warnings and choices.
#include "CStringHelpers.h"						// For AddTrailingBackslash()
#include "CFileHelpers.h"							// For bFileExists() etc
...
//-------------------------------------------------------------------//
bool BaseApp::VerifyInstallation(
	...
	// We will either be starting the instances thread object up to 
	// wait for notifications, or telling it to send notifications 
			...
			if ( pRegData->GetAppPath( &m_strRegistryPath ) )
			{
				...
				if ( GetShortFilename( m_strRegistryPath ) == GetShortFilename( m_strCurrentPath ) )
				{
					...
					if ( bForceOneInstance )
					{
						...
						// Start the instance notification thread.
						m_OtherInstancesThread.Start();
		...
		if ( GetShortFilename( m_strRegistryPath ) != GetShortFilename( m_strCurrentPath ) )
		{
			...
			if ( dwThisVersion > dwInstallVersion )
			{
				...
				if (
					...
					// Force the other instance to shut down, waiting
					// until it does so.
					m_OtherInstancesThread.ForceOtherInstanceShutdown();
			...
			m_OtherInstancesThread.NotifyOtherInstance();
...
// BEGIN_MESSAGE_MAP()																//
//-------------------------------------------------------------------//
BEGIN_MESSAGE_MAP(BaseApp, inherited)
	//{{AFX_MSG_MAP(BaseApp)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
