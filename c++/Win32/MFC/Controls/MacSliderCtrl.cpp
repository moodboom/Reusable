...
COLORREF CMacSliderCtrl::LightenColor(const COLORREF crColor, UINT nIncreaseVal){
...
COLORREF CMacSliderCtrl::DarkenColor(const COLORREF crColor, UINT nReduceVal)
{
	BYTE byRed = GetRValue(crColor);
	BYTE byGreen = GetGValue(crColor);
	BYTE byBlue = GetBValue(crColor);
...
//
{
	...
	if (IsWindowEnabled())
	{
		...
		int nToMid = nWidth / 2;
		pDC->SelectObject(&m_penThumbLight);
		pDC->MoveTo( rc.left + nToMid		, rc.top		+ 4);
		pDC->LineTo( rc.left + nToMid		, rc.bottom - 5);
		pDC->SelectObject(&m_penThumbDarker);
		pDC->MoveTo( rc.left + nToMid + 1, rc.top		+ 5);
		pDC->LineTo( rc.left + nToMid + 1, rc.bottom - 4);
...
//-------------------------------------------------------------------//
void CMacSliderCtrl::OnSize(UINT nType, int cx, int cy) 
{
	// Force the style.
