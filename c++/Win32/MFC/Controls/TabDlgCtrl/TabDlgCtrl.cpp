// TabDlgCtrl.cpp : implementation file
//

#include "stdafx.h"
#include "TabDlgCtrl.h"
#include ".\tabdlgctrl.h"
...
TabDlgCtrl::~TabDlgCtrl (void) {
}

BEGIN_MESSAGE_MAP(TabDlgCtrl, CTabCtrl)
	//{{AFX_MSG_MAP(TabDlgCtrl)
	ON_WM_DESTROY ()
	ON_WM_SETFOCUS ()
	ON_WM_KILLFOCUS ()
...
int TabDlgCtrl::AddTabPage (LPCTSTR pszTitle, int nPageID, TabDlgCtrlPage* pTabPage, int nImageIndex ) 
{
    ...
    return AddPage (pszTitle, nPageID, tabDelete, nImageIndex );
}
...
// in FontDlg class...
/*
int TabDlgCtrl::AddTabPage (LPCTSTR pszTitle, int nPageID, LPCTSTR pszTemplateName) {
	// Verify that the dialog template is compatible with TabDlgCtrl
	...
	TabDlgCtrlPage* pDialog = new TabDlgCtrlPage;
...
int TabDlgCtrl::AddTabPage (LPCTSTR pszTitle, int nPageID, int nTemplateID) {
	return AddTabPage (pszTitle, nPageID, MAKEINTRESOURCE (nTemplateID));
}
*/


BOOL TabDlgCtrl::RemoveSSLPage (int nIndex) {
    ...
    TabDlgCtrlPage* pDialog = tabDelete.pTabPage;
	if (pDialog != NULL) {
		pDialog->DestroyWindow ();	
		delete pDialog;
	}
...
int TabDlgCtrl::GetSSLPageCount (void) {
	return GetItemCount ();
}

BOOL TabDlgCtrl::GetSSLPageTitle (int nIndex, CString &strTitle) {
	if (nIndex >= GetItemCount ())
		return FALSE;
...
BOOL TabDlgCtrl::SetSSLPageTitle (int nIndex, LPCTSTR pszTitle) {
	if (nIndex >= GetItemCount ())
		return FALSE;
...
int TabDlgCtrl::GetSSLPageID (int nIndex) {
	if (nIndex >= GetItemCount ())
		return -1;
...
int TabDlgCtrl::SetSSLPageID (int nIndex, int nPageID) {
	if (nIndex >= GetItemCount ())
		return -1;
...
BOOL TabDlgCtrl::ActivateSSLPage (int nIndex) {
	if (nIndex >= GetItemCount ())
		return FALSE;
...
int TabDlgCtrl::GetSSLActivePage (void) {
	return GetCurSel ();
}

CWnd* TabDlgCtrl::GetSSLPage (int nIndex) {
	if (nIndex >= GetItemCount ())
		return NULL;
...
int TabDlgCtrl::GetSSLPageIndex (int nPageID) {
	int nCount = GetItemCount ();
	if (nCount == 0)
		return -1;
...
#ifdef _DEBUG
BOOL TabDlgCtrl::CheckDialogTemplate (LPCTSTR pszTemplateName) {
...
void TabDlgCtrl::ResizeDialog (int nIndex, int cx, int cy) {
	if (nIndex != -1) {
        TabDelete tabDelete = m_tabs[nIndex];
        TabDlgCtrlPage* pDialog = tabDelete.pTabPage;
...
BOOL TabDlgCtrl::OnInitPage (int nIndex, int nPageID) {
	// TODO: Override in derived class to initialise pages.
	return TRUE;
}

void TabDlgCtrl::OnActivatePage (int nIndex, int nPageID) {
	// TODO: Override in derived class to respond to page activations.
}

void TabDlgCtrl::OnDeactivatePage (int nIndex, int nPageID) {
	// TODO: Override in derived class to respond to page deactivations.
}

void TabDlgCtrl::OnDestroyPage (int nIndex, int nPageID) {
...
void TabDlgCtrl::OnSelChanging (NMHDR* pNMHDR, LRESULT* pResult) {
...
void TabDlgCtrl::OnSelChange (NMHDR* pNMHDR, LRESULT* pResult) {
	int nIndex = GetCurSel ();
	if (nIndex == -1)
		return;
...
void TabDlgCtrl::OnSetFocus (CWnd* pOldWnd) {
...
void TabDlgCtrl::OnKillFocus (CWnd* pNewWnd) {
...
void TabDlgCtrl::OnDestroy (void) {
	int nCount = m_tabs.GetSize ();

	// Destroy dialogs and delete TabDlgCtrl objects.
	if (nCount > 0) {
...
BOOL TabDlgCtrl::OnCommand (WPARAM wParam, LPARAM lParam) {
	// Forward WM_COMMAND messages to the dialog's parent.
	return GetParent ()->SendMessage (WM_COMMAND, wParam, lParam);
}

BOOL TabDlgCtrl::OnNotify (WPARAM wParam, LPARAM lParam, LRESULT* pResult) {
	// Forward WM_NOTIFY messages to the dialog's parent.
	return GetParent ()->SendMessage (WM_NOTIFY, wParam, lParam);
}

BOOL TabDlgCtrl::OnCmdMsg (UINT nID, int nCode, void* pExtra,
...
int TabDlgCtrl::AddPage (LPCTSTR pszTitle, int nPageID, TabDelete tabDelete, int nImageIndex ) 
{
	// Add a page to the tab control.
	TC_ITEM item;
	item.mask = TCIF_TEXT;
...
void TabDlgCtrl::OnSize(UINT nType, int cx, int cy)
{
	inherited::OnSize(nType, cx, cy);

	int nIndex = GetCurSel();
	if ( nIndex != -1 && m_tabs.GetSize() ) 
	{
		TabDelete tabDelete = m_tabs[nIndex];
		TabDlgCtrlPage* pDialog = tabDelete.pTabPage;
