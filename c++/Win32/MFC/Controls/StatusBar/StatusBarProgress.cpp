...
IMPLEMENT_DYNCREATE(StatusBarProgress, CProgressCtrl)

BEGIN_MESSAGE_MAP(StatusBarProgress, CProgressCtrl)
	//{{AFX_MSG_MAP(StatusBarProgress)
	ON_WM_ERASEBKGND()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
StatusBarProgress::StatusBarProgress(LPCTSTR strMessage, int nSize /*=100*/, 
...
CStatusBar* StatusBarProgress::GetStatusBar()
{
...
BOOL StatusBarProgress::Create(LPCTSTR strMessage, int nSize /*=100*/, 
						  int MaxValue /*=100*/, BOOL bSmooth /*=FALSE*/, 
                          int nPane /*=0*/)
{
	BOOL bSuccess = FALSE;
...
BOOL StatusBarProgress::SetText(LPCTSTR strMessage)
{ 
	m_strMessage = strMessage; 
	return Resize(); 
}

BOOL StatusBarProgress::SetSize(int nSize)
{
	m_nSize = nSize; 
	return Resize();
}

COLORREF StatusBarProgress::SetBarColour(COLORREF clrBar)
{
...
COLORREF StatusBarProgress::SetBkColour(COLORREF clrBk)
{
#ifdef PBM_SETBKCOLOR
	if (!IsWindow(GetSafeHwnd()))
		return CLR_DEFAULT;
...
BOOL StatusBarProgress::SetRange(int nUpper, int nLower, int nStep)	
{	 
	if (!IsWindow(GetSafeHwnd()))
		return FALSE;
...
int StatusBarProgress::SetPos(int nPos)	   
{
	if (!IsWindow(GetSafeHwnd()))
		return 0;
...
int StatusBarProgress::OffsetPos(int nPos) 
{ 
	if (!IsWindow(GetSafeHwnd()))
		return 0;
...
int StatusBarProgress::SetStep(int nStep)
{ 
	if (!IsWindow(GetSafeHwnd()))
		return 0;
...
BOOL StatusBarProgress::OnEraseBkgnd(CDC* pDC) 
{
	Resize();
