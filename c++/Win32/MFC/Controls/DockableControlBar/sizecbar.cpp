...
LRESULT CSizingControlBar::OnSetText(WPARAM wParam, LPARAM lParam)
{
        ...
        GetParentFrame()->IsKindOf(RUNTIME_CLASS(CMiniDockFrameWnd)))
    {
		...
		// MDM	4/26/2004 3:13:08 PM
		// If embedded, we won't ever get the delayed recalc...
      // GetParentFrame()->DelayRecalcLayout(); // refresh miniframe
        GetParentFrame()->RecalcLayout(); // refresh miniframe
    }
...
CSize CSizingControlBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
	...
	// MDM 
	// ------------------------------------------------------- //
	// Here, if this is the last row, force the height to 
	// match the remaining space.

	// TO DO
	// Revamp all this - it doesn't consider any changes in order of the control bars!!

	int nA;
	CSizingControlBar* pBar;
    
	// Get the "normal" height.
	int nHeight = bHorz? max(m_szMinHorz.cy, m_szHorz.cy) : max(m_szMinVert.cx, m_szVert.cx);

	int nBars = m_pDockBar->m_arrBars.GetSize();
   int nFirst, nLast, nThis;
   GetRowInfo(nFirst, nLast, nThis);

	// m_arrBars contains a 0 "marker" at the end of each row that we have to 
	// account for.  This means that the bar count will be two more than the index for
	// the last bar.
	bool bLastRow = ( nBars == nLast + 2 );

	// We also have to account for bars that are hidden.
	if ( !bLastRow )
	{
		// Before we can be sure that we are NOT at the last row,
		// we need to check the bar array to see if all bars
		// beyond us are hidden.  To do so, we walk backwards
		// from the last bar to us.  If all are hidden, then
		// we ARE the last.
		for ( nA = nLast + 1; nA < nBars - 1; nA++ )
		{
			pBar = (CSizingControlBar*) m_pDockBar->m_arrBars[nA];
			if ( HIWORD(pBar) != 0 && pBar->IsVisible() )	// placeholder
				break;
		}
		bLastRow = ( nA == nBars - 1 );
	}
	...
	if ( bLastRow )
	{
		nHeight = bHorz ? rc.Height() - 2 : rc.Width() - 12 /*+ 2*/;
		
		// Now we need to reduce nHeight by the amount taken up by other bars.
		bool bNewRow = true;
		for ( nA = 0; nA < nFirst - 1; nA++ )
		{
		    pBar = (CSizingControlBar*) m_pDockBar->m_arrBars[nA];

			if ( HIWORD(pBar) == 0 )	// placeholder
			{
				bNewRow = true;
				continue;
			}
			...
			if ( bNewRow )
			{
				...
				nHeight -= bHorz? rect.Height() : rect.Width();
			}
			bNewRow = false;
		}
...
void CSizingControlBar::StartTracking(UINT nHitTest, CPoint point)
{
    ...
    if (!IsSideTracking())
    {
        ...
        // MDM 
		// ------------------------------------------ //
		// We want to allow oversizing beyond the client area here, 
		// we will fix it in CalcFixedSize().
        BOOL bTopOrLeft = m_htEdge == HTTOP || m_htEdge == HTLEFT;
        m_nTrackPosMin -= bTopOrLeft ? 32500 : nExcessWidth;
        m_nTrackPosMax += bTopOrLeft ? nExcessWidth : 32500;
...
void CSizingControlBar::OnTrackUpdateSize(CPoint& point)
{
    ...
    else
    {
        ...
        for (int i = nFirst; nDelta != 0 && i != nLimit; i += (bBefore ? -1 : 1))
        {
            ...
            int nDeltaT = min(
					nDelta,
					(int) ( (bHorz ? pBar->m_szHorz.cx : pBar->m_szVert.cy) - (bHorz ? pBar->m_szMinHorz.cx : pBar->m_szMinVert.cy) )
				);
...
// NegotiateSpace()																	//
//-------------------------------------------------------------------//
// Here, we size all the bars in our row.
//
//		1) Call GetRowInfo to determine the index of the first and
//			last bar, as well as the index of US.
//		2) Loop through the bars in the row.
//		3) Get the min length.
//			If the bar is sizable (bIsSizingBar), use m_szMin(Horz/Vert)
//			If not, use its current length.
//		4) Add the min bar length to the running total for the row.
//		5) If we've used up all space...
//				If we didn't reach ourselves...
//					Insert a NULL bar after the current bar and return FALSE
//				If we are right at ourselves...
//					Set m_sz(Horz/Vert) to m_szMin(Horz/Vert) and return TRUE
//				We are past ourselves.
//				Insert a NULL bar before the current bar.
//				Change last bar to the last that fit.
//				Break out of loop.
//		6) If we haven't used up space yet, 
...
BOOL CSizingControlBar::NegotiateSpace(int nLengthTotal, BOOL bHorz)
{
	...
	for ( i = nFirst; i <= nLast; i++ )
    {
        ...
        if (bIsSizingBar)
        {
            ...
            nWidthMax = max(
					nWidthMax, 
					(int)( bHorz ? pBar->m_szHorz.cy : pBar->m_szVert.cx )
				);
        }
        else
...
void CSizingControlBar::LoadState(LPCTSTR lpszProfileName)
{
    ...
    m_szHorz.cx = max( (int)m_szMinHorz.cx, (int) pApp->GetProfileInt(
        szSection, _T("sizeHorzCX"), m_szHorz.cx));
    m_szHorz.cy = max( (int)m_szMinHorz.cy, (int) pApp->GetProfileInt(
        szSection, _T("sizeHorzCY"), m_szHorz.cy));

    m_szVert.cx = max( (int)m_szMinVert.cx, (int) pApp->GetProfileInt(
        szSection, _T("sizeVertCX"), m_szVert.cx));
    m_szVert.cy = max( (int)m_szMinVert.cy, (int) pApp->GetProfileInt(
        szSection, _T("sizeVertCY"), m_szVert.cy));

    m_szFloat.cx = max((int)m_szMinFloat.cx, (int) pApp->GetProfileInt(
        szSection, _T("sizeFloatCX"), m_szFloat.cx));
    m_szFloat.cy = max((int)m_szMinFloat.cy, (int) pApp->GetProfileInt(
