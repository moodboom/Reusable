...
{
	if(!m_bDynSplit)
		CGfxSplitterWnd::OnInvertTracker(rect);
	else
		return;
}
...
void CDynSplitterWnd::StartTracking(int ht)
{
	if(!m_bDynSplit)
	{
		CGfxSplitterWnd::StartTracking(ht);
		return;
	}
...
void CDynSplitterWnd::StopTracking(BOOL bAccept)
{
	...
	if ( (pOldActiveView == GetActivePane()) && (pOldActiveView != NULL) )
		SetActivePane(-1, -1, pOldActiveView);
	// re-activate
}


BEGIN_MESSAGE_MAP(CDynSplitterWnd, CGfxSplitterWnd)
...
void CDynSplitterWnd::OnMouseMove(UINT nFlags, CPoint pt)
{
	if(!m_bDynSplit)
	{
		CGfxSplitterWnd::OnMouseMove(nFlags, pt);
		return;
	}
	...
	if (m_bTracking)
	{
		...
		{
			// OLD:
			// This was just plain ol' sloppy-assed.  :>
			/*
