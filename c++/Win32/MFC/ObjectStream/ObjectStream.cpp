...
const uShort uExpectedEHSize = 
	...
	+	sizeof ObjectStreamHeader;		// for DBHeader;
...
// ~ObjectStream()																	//
//-------------------------------------------------------------------//
// ---------- close the ObjectStream database
//-------------------------------------------------------------------//
ObjectStream::~ObjectStream()
{
...
//
//-------------------------------------------------------------------//
DB_INIT_RESULT ObjectStream::Initialize(
	...
	int nDataInitResult = datafile.Initialize( 
		...
		this							// ObjectStream pointer, provides extended header functions
	);
...
// a separate function.
//-------------------------------------------------------------------//
bool ObjectStream::UpdateDatabaseAsNeeded()
{
...
// This opens the file.
//-------------------------------------------------------------------//
bool ObjectStream::Open( bool bWritable )
{ 
...
// Close()																				//
//-------------------------------------------------------------------//
void ObjectStream::Close( bool bWritable )
{
...
//-------------------------------------------------------------------//
void ObjectStream::ReadPlainHeader()
{

	// Read the size of the ObjectStream plain header.
	datafile.ReadData(
...
// header version.
//-------------------------------------------------------------------//
bool ObjectStream::ReadEncryptedHeader()
{

	bool bReturn = true;

	// Read the size of the ObjectStream encrypted header.
	datafile.ReadData(
...
// unencrypted data.
//-------------------------------------------------------------------//
void ObjectStream::WritePlainHeader()
{
...
// encrypted data.
//-------------------------------------------------------------------//
void ObjectStream::WriteEncryptedHeader()
{
	...
	if ( uObjectStreamEncryptedHeaderSize == uExpectedEHSize ) {
		...
		ObjectStreamHeader EncryptedHeader;
		memcpy(
			&EncryptedHeader,
			&DBHeader,
			sizeof DBHeader
...
//-------------------------------------------------------------------//
/* 
bool ObjectStream::GetCacheStatus( ClassID CID )
{
...
void ObjectStream::SetCacheStatus( 
	ClassID	CID,
...
//-------------------------------------------------------------------//
/*
uHuge ObjectStream::GetCurrentFileVersion() 
{
...
// return -1.
//-------------------------------------------------------------------//
ClassID ObjectStream::GetClassID( const Persistent* pObject )
{
...
//-------------------------------------------------------------------//
Class* ObjectStream::GetClass( ClassID CID )
{
...
// class.
//-------------------------------------------------------------------//
bool ObjectStream::FindClass(Class *cls, NodeNbr *nd)
{
...
// AddClassToIndex()																	//
//-------------------------------------------------------------------//
void ObjectStream::AddClassToIndex(
	Class *cls
) {
...
//-------------------------------------------------------------------//
void ObjectStream::RegisterIndexes(Class *cls, const Persistent& pcls)
			throw (ZeroLengthKey)
{
...
//-------------------------------------------------------------------//
void ObjectStream::RegisterClass(
...
//-------------------------------------------------------------------//
bool ObjectStream::RegisterClasses()
{
...
// not deleted.
//-------------------------------------------------------------------//
int ObjectStream::GetPercentCompacted()
{
...
// ObjectStream that has defined the abstract NewObject() function.
//-------------------------------------------------------------------//
void ObjectStream::RebuildIndexFile( 
...
// if not.
//-------------------------------------------------------------------//
bool ObjectStream::VerifyFilesAreWritable( 
	bool	bNoPrompt,
	UINT	MessageID 
) {
...
//-------------------------------------------------------------------//
bool ObjectStream::Backup( bool bAddVersion )
{
...
//-------------------------------------------------------------------//
void ObjectStream::GetDatabaseHeader( CString* pHeader )
{
...
// SetProgressTitle																	//
//-------------------------------------------------------------------//
void ObjectStream::SetProgressTitle(
