...
protected:
  ...
  const string &getStaticFile(const string &path) { return static_files_[path]; }
...
inline void HttpAPIServer::add_static_file_handlers(const vector<string> &static_files, const string &prefix)
{
  ...
  for (auto &file : static_files)
  {
    ...
    {
      const string &body = getStaticFile(request->path);
      *response << cstr_HTML_HEADER1 << body.length() << contentType(request->path) << cstrLongCache << cstr_HTML_HEADER2 << body;
    };
  }
}

inline string HttpAPIServer::contentType(const string &path)
{
  ...
  string content_type = "\r\nContent-Type: ";
  if (b_string_ends_in(path, ".html"))
    content_type += "text/html";
  else if (b_string_ends_in(path, ".css"))
    content_type += "text/css";
  else if (b_string_ends_in(path, ".js"))
    content_type += "text/javascript";
  else if (b_string_ends_in(path, ".png"))
    content_type += "image/png";
  else if (b_string_ends_in(path, ".jpg"))
    content_type += "image/jpeg";
  else if (b_string_ends_in(path, ".svg"))
    content_type += "image/svg+xml";
  else if (b_string_ends_in(path, ".ico"))
...
inline bool HttpAPIServer::tokenize_API_querystring(const std::string &querystring, API_call &ac)
{
  ...
  for (tokenizer::iterator tok_iter = tokens.begin(); tok_iter != tokens.end(); ++tok_iter)
  {
    ...
    tokenizer::iterator value_iter = key_iter;
    ++value_iter;
