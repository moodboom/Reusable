...
// QUICKSTART:
//
//      std::function login_handler = ....
//      std::function analyze_handler = ....
//      HttpsAPIServer api_server;
//      api_server.vAPI_ = { 
//          API_call(login_handler    , HM_GET, {"v1","login"                      }, {"html"       }),
//          API_call(analyze_handler  , HM_GET, {"v1","accounts",":id","analyze"   }, {"html"       }) 
//      };
...
public:
    ...
    API_call(
        std::function<void(std::shared_ptr<HttpsServer::Response>, std::shared_ptr<HttpsServer::Request>)> handler,
        HTML_METHOD method,
    ...
    // Define this function to do the work when the corresponding call comes in.  C++11 ftw.
    // NOTE that this acts like any other object!  If you provide an existing function, we make a FULL COPY.
    // You can use std::move when building API_call to nicely let go of your function and move it in here: 
    //
    //      API_call(std::move(myFunction), ...)
    //
    std::function<void(std::shared_ptr<HttpsServer::Response>, std::shared_ptr<HttpsServer::Request>)> handler_;
    // ----------------------
...
public:
    ...
    //      (your handlers) -> loaded from vAPI_ (make sure to define it before starting server!)
...
inline void HttpsAPIServer::registerRESTHandlers() {
    // Make sure you have provided an API before calling startServer()!
    assert(vAPI_.size() > 0);

    for (auto& call : vAPI_)
    {
        resource[call.regex()][call.method()]= call.handler_;
    }
}
    ...
    bool process_API_call(API_call& ac, ServerBase<HTTPS>::Response& rep)
    {
        for (auto& acloop : vpAPI_)
        {
            if (b_API_call_matches(*acloop,ac))
            {
                ...
                {
                    // THIS WILL NOW BE DONE DIFFERNETLY in SWS
