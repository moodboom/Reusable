...
inline void HttpsAPIServer::load_index()
{
    try
    {
...
// Called on startup using a constructor parameter
inline void HttpsAPIServer::set_html_includes(const vector<string>& includes)
{
...
// Called on startup for each preloaded html file.
// Search for any include files, and replace with the full script or style.
inline void HttpsAPIServer::inject_includes(API_call& ac)
{
...
inline string HttpsAPIServer::get_API_html()
{
    string html;
    for (auto& ac : vAPI_)
    {
...
inline string HttpsAPIServer::build_path(const string& param)
{
...
inline string HttpsAPIServer::build_param(const string& param)
{
    // Substitute var as needed.
