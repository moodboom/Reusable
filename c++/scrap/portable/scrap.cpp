...
int main( int argc, char * argv[] )
{
  ...
  cout << endl << "== 4 === hash ==========" << endl;
    ...
    BOOST_FOREACH(hashmap::value_type i, x) {
  ...
  cout << endl << "== 10 === storage vs speed" << endl;
    ...
    for (int n = 0; n < 100000000; n++)
    {
        ...
        b_has_a = (bit_ephases|PHASE_A_MASK) == PHASE_A_IN_A || (bit_ephases|PHASE_B_MASK) == PHASE_A_IN_B || (bit_ephases|PHASE_C_MASK) == PHASE_A_IN_C;
        b_has_ac = 
                ((bit_ephases|PHASE_A_MASK) == PHASE_A_IN_A || (bit_ephases|PHASE_B_MASK) == PHASE_A_IN_B || (bit_ephases|PHASE_C_MASK) == PHASE_A_IN_C)
            &&  ((bit_ephases|PHASE_A_MASK) == PHASE_C_IN_A || (bit_ephases|PHASE_B_MASK) == PHASE_C_IN_B || (bit_ephases|PHASE_C_MASK) == PHASE_C_IN_C);
    ...
    for (int n = 0; n < 100000000; n++)
    {
        ...
        // int n_init_phase = rand() % 7;
        byte_phases[0] = 1; byte_phases[1] = 1; byte_phases[2] = 1; 
        b_has_a = byte_phases[0] == 0;
        b_has_ac = (byte_phases[0] == 0) && (byte_phases[2] == 0);
        byte_phases[0] = 1; byte_phases[1] = 0; byte_phases[2] = 1; 
        byte_phases[1] = 1;
        ...
        b_has_ac = ((byte_ephases[0] == PHASE_A) || (byte_ephases[1] == PHASE_A) || (byte_ephases[2] == PHASE_A)) && ((byte_ephases[0] == PHASE_C) || (byte_ephases[1] == PHASE_C) || (byte_ephases[2] == PHASE_C));
    }
